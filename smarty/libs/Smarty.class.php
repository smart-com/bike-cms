<?php
	...
	class Smarty extends Smarty_Internal_TemplateBase {
		...
		public function addPluginsDir( $plugins_dir ) {
			...
			$this->plugins_dir = array_merge( ( array ) $this->plugins_dir,
									 ( array ) $plugins_dir );
			if( isset( $this->_cache[ 'plugins_dir' ] ) ) {
				unset( $this->_cache[ 'plugins_dir' ] );
			}
		...
		public function createTemplate( $template, $cache_id = null,
								  $compile_id = null, $parent = null, $do_clone = true ) {
			...
			if( $this->caching && isset( $this->_cache[ 'isCached' ][ $_templateId = $this->_getTemplateId( $template,
																								   $cache_id, $compile_id ) ] ) ) {
				...
				$tpl = new $this->template_class( $template, $this, $parent, $cache_id,
									  $compile_id, null, null );
			}
			if( $do_clone ) {
		...
		public function loadPlugin( $plugin_name, $check = true ) {
			return Smarty_Internal_Extension_LoadPlugin::loadPlugin( $this, $plugin_name,
															$check );
		}
		 ...
		 * @return string
		 */
		public function _getTemplateId( $template_name, $cache_id = null,
								  $compile_id = null ) {
		...
		public static function mutingErrorHandler( $errno, $errstr, $errfile,
			...
			if( !$_is_muted_directory || ($errno && $errno & error_reporting()) ) {
				if( Smarty::$_previous_error_handler ) {
					return call_user_func( Smarty::$_previous_error_handler, $errno, $errstr,
